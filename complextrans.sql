...
/* CT 1.2 อ้างอิงด้วย Payment_Id หลังจากได้ Book_Price ใน All_Book */
...
/* CT 3 */
SELECT h.Hotel_Id, h.Hotel_Name, AVG(t.Rating_Score) as Rating_Avg
	FROM `transaction` t, `allbook` a, `room` r, `hotel` h
	WHERE h.Hotel_Id LIKE 'H01' AND
	h.Hotel_Id LIKE r.Hotel_Id AND
	r.Room_Id LIKE a.Room_Id AND
	a.Transaction_Id LIKE t.Transaction_Id;
